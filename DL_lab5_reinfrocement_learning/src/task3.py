...
#python task3.py --wandb-run-name "task3.py" --num-episodes 5000 --batch-size 64 --memory-size 100000 --lr 0.0001 --discount-factor 0.99 --epsilon-start 1.0 --epsilon-min 0.05 --target-update-frequency 10000 
# --replay-start-size 50000 --max-episode-steps 10000 --train-per-step 1 --seed 42 --epsilon-decay-steps 1000000 --train-per-step 4 --student-id 109208063 --n-step 4 
...
# --- Prioritized Replay Buffer with Multi-Step Return ---
class PrioritizedReplayBuffer:
    ...
    def _get_n_step_transition(self):
        ...
        for idx, transition in enumerate(list(self.n_step_buffer)[1:-1], start=1):
            r, _, d = transition[2:]
    ...
    def add(self, state, action, reward, next_state, done):
        ...
        state, action, reward, next_state, done = self._get_n_step_transition()
...
class DQNAgent:
    ...
    def train(self):
        ...
        with torch.no_grad():
            ...
            target_q = rewards + (1 - dones) * (self.gamma ** self.memory.n_step) * next_q_values
        ...
        if self.total_steps in self.model_save_steps and self.total_steps not in self.saved_checkpoints:
            filename = f"LAB5_{self.student_id}_task3_pong{self.total_steps}.pt"
            save_path = os.path.join(self.save_dir, filename)
            torch.save(self.q_net.state_dict(), save_path)
            print(f" Saved checkpoint at step {self.total_steps} → {save_path}")
            self.saved_checkpoints.add(self.total_steps)
    def run(self, num_episodes=10000):
        self.saved_checkpoints = set()
        self.model_save_steps = [200000, 400000, 600000, 800000, 1000000,1200000]  # 可視需要調整
        ...
        for episode in range(num_episodes):
            ...
            print(f"Episode {episode} - Total Reward: {total_reward:.2f} - Epsilon: {self.epsilon:.3f} - Steps: {self.total_steps}")
            wandb.log({
                "Episode": episode,
    ...
    def evaluate(self, episodes=10):
        ...
        for _ in range(episodes):
            obs, _ = self.test_env.reset(seed=self.seed)
            state = self.preprocessor.reset(obs)
            total_reward = 0
            done = False
        ...
        avg_reward /= episodes
        wandb.log({"Evaluation Avg Reward": avg_reward}, step=self.total_steps)

        if avg_reward > self.best_eval_reward:
            self.best_eval_reward = avg_reward
            model_path = os.path.join(self.save_dir, f"pong_best_model_{self.total_steps}.pt")
            torch.save(self.q_net.state_dict(), model_path)
            print(f" New best model saved at step {self.total_steps} with avg reward {avg_reward:.2f}")
